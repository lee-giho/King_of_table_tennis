...
- [📈 서버 안정성 개선](#-서버-안정성-개선)
- [🛠 트러블슈팅 🏗](#-트러블슈팅-)
  - [**경기 만료 처리 누락**으로 오래된 `모집중`, `경기 대기중`인 경기가 조회 목록에 남는 문제](#경기-만료-처리-누락으로-오래된-모집중-경기-대기중인-경기가-조회-목록에-남는-문제)
  - [채팅 메시지 전송 직후 WebSocket으로 프리뷰를 보내던 구조에서 **커밋 이전 데이터가 전송될 수 있는 문제**](#채팅-메시지-전송-직후-websocket으로-프리뷰를-보내던-구조에서-커밋-이전-데이터가-전송될-수-있는-문제)
- [🛠 핵심 기능 및 UI 🏗](#-핵심-기능-및-ui-)
  - [주요 내용](#주요-내용)
    - [경기 매칭 시스템](#경기-매칭-시스템)
...
# 🛠 트러블슈팅 🏗
## **경기 만료 처리 누락**으로 오래된 `모집중`, `경기 대기중`인 경기가 조회 목록에 남는 문제
- 정상적으로 종료되지 않은 상태를 의미하는 `EXPIRED`, 만료 상태 도입 + 목록 조회 시 **비동기 만료 처리 트리거**
- **60초 쓰로틀링/CAS 경합 방지**로 중복 업데이트를 최소화했습니다.

## 채팅 메시지 전송 직후 WebSocket으로 프리뷰를 보내던 구조에서 **커밋 이전 데이터가 전송될 수 있는 문제**
- `AFTER_COMMIT` 이벤트 리스너로 전송 시점 분리
- 커밋 이후 **Projection 재조회**로 **DB와 UI 간 데이터의 정합성과 일관성** 보장

<br>

🔎 자세한 내용: **[Troubleshooting 문서 보기](./Troubleshooting.md)**
